...
class SystemPromptCompiler:
    ...
    def compile(self, identity: AgentIdentity, format: str = "text") -> str:
        ...
        board_section = self._render_board(identity.board)
        if board_section:
            named_sections.append(("board", board_section))
    ...
    def _wrap_anthropic(self, prompt: str, identity: AgentIdentity) -> str:
        ...
        section_map = {
            ...
            "## Historical Advisory Board": "board",
        }
...
class OpenClawCompiler:
    ...
    def compile(
        ...
        # Include board config if enabled
        if identity.board.enabled:
            board_data: dict[str, Any] = {
                "enabled": True,
                "disclaimer": identity.board.disclaimer,
                "board_members": [
                    {
                        "id": m.id,
                        "name": m.name,
                        ...
                        "core_mindset": m.core_mindset,
                        "modern_relevance": m.modern_relevance,
                        "personality": {
                            "ocean": m.personality.ocean.model_dump(),
                            "disc_style": m.personality.disc_style.value,
                            "jungian_type": m.personality.jungian_type,
                        },
                    }
                    for m in identity.board.board_members
                ],
                "engagement_rules": identity.board.engagement_rules,
            }
            if identity.board.notes:
                board_data["notes"] = identity.board.notes
...
class SoulCompiler:
    ...
    def _render_soul(self, identity: AgentIdentity) -> str:
        ...
        board = self._soul_board(identity)
        if board:
...
class MarkdownCompiler:
    ...
    def compile(self, identity: AgentIdentity) -> str:
        ...
        # Board
        if identity.board.enabled:
            board_lines = ["## Historical Advisory Board"]
            if identity.board.disclaimer:
                board_lines.append(f"\n> {identity.board.disclaimer}")
            if identity.board.board_members:
                for member in identity.board.board_members:
                    board_lines.append(f"\n**{member.name}** ({member.died}) — {member.board_role}")
                    board_lines.append(f"- Core mindset: {member.core_mindset}")
                    board_lines.append(f"- Modern relevance: {member.modern_relevance}")
            if identity.board.engagement_rules:
                board_lines.append("\n**Engagement Rules:**")
                for rule in identity.board.engagement_rules:
                    board_lines.append(f"- {rule}")
            if identity.board.notes:
                board_lines.append(f"\n_{identity.board.notes}_")
            sections.append("\n".join(board_lines))
