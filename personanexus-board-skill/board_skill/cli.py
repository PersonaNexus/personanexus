"""CLI for the PersonaNexus Board Skill."""
...
from board_skill.compiler import CompilerError, compile_identity
from board_skill.parser import ParseError, parse_identity_file
from board_skill.types import TRAIT_ORDER
from board_skill.validator import IdentityValidator
...
def personality_ocean_to_traits(
    ...
    from board_skill.personality import ocean_to_traits
    from board_skill.types import OceanProfile
...
def personality_disc_to_traits(
    ...
    from board_skill.personality import disc_to_traits, get_disc_preset
    from board_skill.types import DiscProfile
...
def personality_jungian_to_traits(
    ...
    from board_skill.personality import get_jungian_preset, jungian_to_traits
    from board_skill.types import JungianProfile
...
def personality_list_disc_presets() -> None:
    ...
    from board_skill.personality import list_disc_presets
...
def personality_list_jungian_presets() -> None:
    ...
    from board_skill.personality import list_jungian_presets
...
board_app = typer.Typer(
    name="board",
    help="Board of Directors configuration utilities",
    add_completion=False,
)
...
def board_show(
    ...
    """Display the board configuration from a YAML identity file."""
    try:
    ...
    if not board.enabled:
        console.print("[dim]Board of Directors is not enabled in this identity file.[/dim]")
        raise typer.Exit()

    table = Table(title="Board Configuration", show_header=True, header_style="bold")
    table.add_column("Field", style="cyan")
    table.add_column("Value")

    table.add_row("Enabled", str(board.enabled))
    table.add_row("Members", str(len(board.board_members)))
    ...
    if board.board_members:
        console.print()
        members_table = Table(title="Board Members", show_header=True, header_style="bold")
        members_table.add_column("#", style="dim")
        members_table.add_column("Name", style="cyan")
        members_table.add_column("Role")
        members_table.add_column("Jungian")
        members_table.add_column("DISC")
        members_table.add_column("Died", style="dim")

        for i, m in enumerate(board.board_members, 1):
            members_table.add_row(
                str(i),
                m.name,
                m.board_role,
                m.personality.jungian_type,
                m.personality.disc_style.value,
                m.died,
            )
    ...
    if board.engagement_rules:
        console.print()
        console.print("[bold]Engagement Rules:[/bold]")
        for rule in board.engagement_rules:
            console.print(f"  - {rule}")


@board_app.command("list-members")
def board_list_members(
    file: Annotated[Path, typer.Argument(help="Path to a YAML identity file")],
) -> None:
    """List all board members with detailed personality info."""
    try:
        identity = parse_identity_file(str(file))
    except (ParseError, ValidationError) as exc:
        console.print(f"[red]Error:[/red] {exc}")
    ...
    if not board.board_members:
        console.print("[dim]No board members defined.[/dim]")
        raise typer.Exit()

    for i, m in enumerate(board.board_members, 1):
        console.print(f"\n[bold cyan]#{i} {m.name}[/bold cyan] ({m.died})")
        console.print(f"  Role: {m.board_role}")
        console.print(f"  Historical Figure: {m.historical_figure}")
        console.print(f"  Mindset: {m.core_mindset}")
        ...
        if m.key_quote:
            console.print(f'  Quote: "{m.key_quote}"')
