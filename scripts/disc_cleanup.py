            ...
            for line_num, line in enumerate(lines, 1):
                ...
                for match in preset_matches:
                    if match not in ["disc_preset", "DISC preset", "DISC presets"]:
                        issues["inconsistent_preset_names"].append(
    ...
    for ext in ["*.md", "*.rst", "*.yaml", "*.yml"]:
        for file_path in root_dir.rglob(ext):
            try:
                ...
                lines = content.split('\n')
                for line_num, line in enumerate(lines, 1):
                    # In docs, prefer "DISC" for user-facing text
                    disc_pat = r'\bdisc\b(?!\s*[_=:])'
                    has_disc = re.search(disc_pat, line, re.IGNORECASE)
                    if has_disc and not re.search(r'DISC', line):
                        issues["doc_inconsistencies"].append(
                            (str(file_path), line_num, line.strip())
                        )
...
def generate_cleanup_report(root_dir: Path) -> str:
    ...
    report.append(
        "4. **Function names**: Use snake_case consistently"
        " (get_disc_preset, list_disc_presets)"
    )
